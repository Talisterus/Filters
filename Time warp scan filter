...
number_of_pixels_to_move_hor=$((("$height"/"$numberoffiles")+1))
...
number_of_pixels_to_move_ver=$((("$width"/"$numberoffiles")+1))
...
#chce zeby wycinek nadpisal sie na oryginalnej klatce i iterowal tak dalej
  ...
  convert $f \( hor_video_base_frame.png -rotate 180 \) \( -stroke cyan -draw "line 0,$i $width,$i" \) -composite "hor_video_"$f
  ...
  convert $f \( ver_video_base_frame.png -rotate 180 \) \( -stroke cyan -draw "line $j,0 $j,$height" \) -composite "ver_video_"$f
...
ffmpeg -i "hor_video_img%04d.png" -i $1 -r:v $framerate -c copy -map 0:v:0 -map 1:a:0? -codec:v libx264 -preset veryslow -pix_fmt yuv420p -crf 28 -an hor_video.mp4
echo "Processing vertical video..."
ffmpeg -i "ver_video_img%04d.png" -i $1 -r:v $framerate -c copy -map 0:v:0 -map 1:a:0? -codec:v libx264 -preset veryslow -pix_fmt yuv420p -crf 28 -an ver_video.mp4
echo "Making temporary token video for testing"
ffmpeg -i "img%04d.png" -i $1 -r:v $framerate -c copy -map 0:v:0 -map 1:a:0? -codec:v libx264 -preset veryslow -pix_fmt yuv420p -crf 28 -an middle_video.mp4
